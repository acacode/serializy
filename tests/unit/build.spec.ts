  ...
  describe(`Test build files (${filePath})`, () => {
    ...
    const funcs: any = {
      ...
      fieldArray: [
        [
          'should be defined',
          () => expect(serializyBuild.fieldArray).toBeDefined()
        ]
      ],
      model: [
        ['should be defined', () => expect(serializyBuild.model).toBeDefined()]
      ]
    }
    ...
    describe('model.serialize()', () => {
      test('Should be possible to serialize structure', () => {
        const { serializedModel, usageStructure } = createModelUsingSpec(
          serializyBuild
        )
    ...
    describe('model.deserialize()', () => {
      test('Should be possible to deserialize structure', () => {
        const { serializedModel, originalStructure } = createModelUsingSpec(
          serializyBuild
        )
...
createBuildFilesTest(
  require('../../dist/serializy.min.js'),
  'dist/serializy.min.js'
)
// tslint:disable-next-line:no-var-requires
